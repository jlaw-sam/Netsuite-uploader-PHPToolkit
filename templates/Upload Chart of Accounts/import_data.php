...
for($i=0; $i<count($addListResponse->writeResponseList->writeResponse); $i++)
{
    ...
    if (!$addListResponse->writeResponseList->writeResponse[$i]->status->isSuccess) {
        ...
        print_r("<strong>Error: </strong> ".$addListResponse->writeResponseList->writeResponse[$i]->status->statusDetail[0]->message."<br>");
    } else {
        ...
        // mysqli_query($conn, $sql);
        echo "<strong>Success: </strong> Created Record With Internal ID " .$response."<br/>";
